...
window.onload = () => {
  ...
  const moveBtn = document.body.querySelector('#move-btn');
  const moveBtnControl = moveBtn.querySelector('.move-control');
  if (!isMobile) {
  ...
  const resize = () => {
    zumaGame.setScale(
      Math.min(window.innerHeight / zumaGame.height, window.innerWidth / zumaGame.width, 1),
    );
  };
